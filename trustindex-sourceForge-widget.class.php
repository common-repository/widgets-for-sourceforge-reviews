...
class TrustindexWidget_sourceForge extends WP_Widget {
...
{
parent::__construct(
'trustindex_sourceForge_widget',
'Trustindex - SourceForge reviews widget',
[
'classname' => 'trustindex-widget',
'description' => 'Embed SourceForge reviews fast and easily into your WordPress site. Increase SEO, trust and sales using SourceForge reviews.'
]
);
}
...
{
global $wpdb;
global $trustindex_pm_sourceForge;
if ($trustindex_pm_sourceForge->is_enabled()) {
extract($args);
echo $before_widget;
$wasError = false;
...
echo $trustindex_pm_sourceForge->renderWidgetFrontend($instance['ti-widget-ID']);
} else if ($trustindex_pm_sourceForge->is_noreg_linked()) {
echo $trustindex_pm_sourceForge->renderWidgetFrontend();
} else {
echo $trustindex_pm_sourceForge->frontEndErrorForAdmins(sprintf(__("Please fill out <strong>all the required fields</strong> in the <a href='%s'>widget settings</a> page", 'trustindex-plugin'), admin_url('admin.php?page='.$trustindex_pm_sourceForge->get_plugin_slug().'/settings.php')));
}
echo $after_widget;
} else {
}
...
global $wp_version;
global $trustindex_pm_sourceForge;
$tiWidgets = $trustindex_pm_sourceForge->get_trustindex_widgets();
...
<?php if ($trustindex_pm_sourceForge->is_trustindex_connected()): ?>
<?php if ($tiWidgets): ?>
...
<?php endforeach; ?>
<?php else: ?>
<?php echo TrustindexPlugin_sourceForge::get_alertbox('warning',
...
<img src="<?php echo $trustindex_pm_sourceForge->get_plugin_file_url('static/img/help-where-is-id.jpg'); ?>" alt="ID column here: <?php echo 'https://admin.trustindex.io/'; ?>widget" />
</div>
<?php else: ?>
<?php echo TrustindexPlugin_sourceForge::get_alertbox('warning',
...
. sprintf(__("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", 'trustindex-plugin'), admin_url('admin.php?page='.$trustindex_pm_sourceForge->get_plugin_slug().'/settings.php&tab=setup_trustindex'))
); ?>
<?php endif; ?>
</div>
<?php
